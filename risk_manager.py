...
def calculate_position_size(price: float) -> float:
    """
    Returns the notional dollar amount to buy, risking at most RISK_PCT of portfolio.

    Formula: portfolio_value * RISK_PCT (minimum $1)
    """
    portfolio_value = get_portfolio_value()
    notional = portfolio_value * settings.RISK_PCT
    return max(round(notional, 2), 1.0)
