...
def run_consensus(state: dict, check_only: bool = False) -> dict:
    ...
    try:
        ...
        qty = meta_plan["position_size_calculation"]["shares_recommended"]
        ...
        if position_plan.get("confidence_adjustment") in ("RR_LOW", "RR_VERY_LOW"):
            ...
            logger.warning(
                ...
                f"halving Kelly qty to {qty}. {position_plan.get('notes', '')}"
            )
    except Exception as exc:
    ...
    logger.info(
        f"CONSENSUS REACHED [{state['symbol']}]: {signal} {qty} shares "
